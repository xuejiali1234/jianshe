    ...
    if observation_level == "movement":
        ...
        for entity_index, movement_id in enumerate(edge_ids):
            ...
            movement_payload: dict[str, Any] = {
                ...
                "lane_ids": meta.get("lane_ids", []),
                "lane_count": meta.get("lane_count", len(meta.get("lane_ids", []))),
                "link_index": meta.get("link_index", -1),
                "link_indexes": meta.get("link_indexes", []),
                "phase_id": meta.get("phase_id", -1),
                "green_phase_ids": meta.get("green_phase_ids", []),
                "incoming_length_m": meta.get("incoming_length_m", 0.0),
                "zone_length_m": meta.get("zone_length_m", 0.0),
                "arrival_start_m": meta.get("arrival_start_m", 0.0),
                "arrival_end_m": meta.get("arrival_end_m", 0.0),
                "queue_start_m": meta.get("queue_start_m", 0.0),
                "queue_end_m": meta.get("queue_end_m", 0.0),
                "upstream_edges": meta.get("upstream_edges", []),
                "zone_edges": meta.get("zone_edges", []),
                "zone_lane_m": meta.get("zone_lane_m", 0.0),
                "zone_quality": meta.get("zone_quality", ""),
            }
